...
class Control:
    ...
    def click(self, x: int | float = 0, y: int | float = 0, specified_hwnd=None):
        ...
        win32gui.PostMessage(
            current_hwnd, win32con.WM_LBUTTONDOWN, win32con.MK_LBUTTON, long_position
        )  # 鼠标左键按下
        time.sleep(0.2)
        win32gui.PostMessage(
            current_hwnd, win32con.WM_LBUTTONUP, win32con.MK_LBUTTON, long_position
        )  # 鼠标左键抬起
        time.sleep(0.1)
    ...
    def left_shift(self):
        self.key_press(win32con.VK_LSHIFT)  # 按下左Shift键
        time.sleep(np.random.uniform(0, 0.01))  # 按键时间 不确定是否需要
        self.key_release(win32con.VK_LSHIFT)  # 松开左Shift键
